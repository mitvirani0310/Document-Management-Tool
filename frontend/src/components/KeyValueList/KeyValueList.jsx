...
// const KeyValueList = ({ data, setData, isRedacted, handleKeyValueClick, isLoading, isRedacting, isRedact, isExtract, handleRedactData, handleExtractData, isSearchable }) => {
...
//             <button className="px-2 py-0.5 mr-2 rounded-md text-lg font-bold bg-blue-600 hover:bg-blue-700 text-white ">
//               +
//             </button>
//              <button
...
const KeyValueList = ({ data, setData, isRedacted, handleKeyValueClick, isLoading, isRedacting, isRedact, isExtract, handleRedactData, handleExtractData, isSearchable }) => {
  ...
  if (isLoading) {
    return (
      <div className={`w-full ${theme === "dark" ? "bg-gray-800" : "bg-white"} rounded-lg shadow-lg p-4 flex flex-col items-center justify-center min-h-[200px]`}>
        <div className="relative flex items-center justify-center">
          <div className="animate-spin rounded-full h-32 w-32 border-t-4 border-b-4 border-blue-500"></div>
          <div className="absolute">
        ...
        {isRedact && (
          <div className="flex items-center">
            <button 
              ...
              className="px-2 py-1.5 mr-2 rounded-md text-lg font-bold bg-blue-600 hover:bg-blue-700 text-white"
            >
          ...
          <tbody className={`${theme === "dark" ? "bg-gray-800" : "bg-white"} divide-y ${theme === "dark" ? "divide-gray-700" : "divide-gray-200"}`}>
            ...
            {Object.entries(data).map(([key, valueObj], index) => {
              ...
              <tr className={`${theme === "dark" ? "bg-gray-700" : "bg-gray-100"}`}>
                {isRedact && <td className="w-12 px-6 py-4"></td>}
                <td className="w-1/3 px-6 py-4">
                  <input
                    ...
                    className="w-full px-2 py-1 rounded-md border text-gray-900"
                  />
                </td>
                <td className="w-2/3 px-6 py-4 flex items-center space-x-2">
                  <input
                    type="text"
                    value={newValue}
                    onChange={(e) => setNewValue(e.target.value)}
                    placeholder="Enter value"
                    className="flex-1 px-2 py-1 rounded-md border text-gray-900"
                  />
                  <button
                    onClick={handleAddNewEntry}
                    className="px-1 py-1 bg-blue-600 text-white rounded hover:bg-blue-800"
                  >
                    <FiCheck size={20}/>
                  </button>
                </td>
                {isExtract && !isRedact && <td className="w-3/3 px-6 py-4"></td>}
              </tr>
            )}
          </tbody>
        </table>
